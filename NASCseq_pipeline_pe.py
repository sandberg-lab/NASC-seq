...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--ensemblID',type=str)
    o = parser.parse_args()
...
if o.flag=='v3conv_tag' or o.flag=='all':
	...
	for cell in glob.glob(os.path.join(o.experimentdir,bamfiles,annotatedsortedfiles,'*/*sorted.bam')):
		...
		cmd = ['python2', os.path.join(rootDir,'scripts/ConvperPos.py'),cell,outfile,outfile2,cellname]
		cmds.append(cmd)
	if verbose: print(" ".join(cmds))
	Parallel(n_jobs=int(o.numCPU))(delayed(run_cmd)(cmd) for cmd in cmds)
	for cmd in cmds:
...
if o.flag=='intronicCounts':
	outfile=os.path.join(o.experimentdir,outfiles,'intronSAF.RDS')
	cmd1=[os.path.join(rootDir,'makeSAF.R'),gtf,outfile]
	run_cmd(cmd1)
	cmds=[]
	safe_mkdir(os.path.join(o.experimentdir,bamfiles,intronannotatefiles))
	for cell in glob.glob(os.path.join(o.experimentdir,bamfiles,removedup,'*/*_removeDupl.bam')):
		cellname='_'.join(os.path.basename(cell).split("_")[:3])
		outpath = os.path.join(o.experimentdir,bamfiles,intronannotatefiles,cellname)
		safe_mkdir(outpath)
		cmd = [os.path.join(rootDir,'scripts/RsubReadFeatureCounts_se_introns.R'),cell, outfile, outpath]
		cmds.append(cmd)
	Parallel(n_jobs=int(o.numCPU))(delayed(run_cmd)(cmd) for cmd in cmds)
	for cmd in cmds:
		commandlogfile.write('%s\n' % cmd)
	print ("BAM file annoated with intronic annotations")

if o.flag=='calculatePiG':
	safe_mkdir(os.path.join(o.experimentdir,outfiles))
	safe_mkdir(os.path.join(o.experimentdir,outfiles,pi_g))
	print ('Calculating Pi-G for all cells that passed filter...')
	cmds=[]
	for cell in glob.glob(os.path.join(o.experimentdir,bamfiles,filterTaggedFiles,'*/*_taggedFiltered.bam')):
		cellname=cellNameExtration(cell)
		if cellListCheck(cellname,o.experimentdir):
			outfile=os.path.join(o.experimentdir,outfiles,pi_g,cellname,cellname+'_pi_g.csv')
			safe_mkdir(os.path.join(o.experimentdir,outfiles,pi_g,cellname))
			if os.path.exists(os.path.join(o.experimentdir,qcfiles,p_e,cellname+'_p_e.txt')):
				pe_file=open(os.path.join(o.experimentdir,qcfiles,p_e,cellname+'_p_e.txt'))
				pe_estimate=pe_file.read().split('\n')[0]
			else:
				pe_estimate=None
			cmd=['python3',os.path.join(rootDir,'scripts/estimate_pi_g.py'),cell,outfile,pe_estimate]
			cmds.append(cmd)
	if verbose: print(" ".join(cmds))
	Parallel(n_jobs=int(o.numCPU))(delayed(run_cmd)(cmd) for cmd in cmds)
	for cmd in cmds:
		commandlogfile.write('%s\n' % cmd)
	print ("Pi-G has been calculated for all cells that passed filter.")


if o.flag=='calculateSpecificPiG':
	safe_mkdir(os.path.join(o.experimentdir,outfiles))
	safe_mkdir(os.path.join(o.experimentdir,outfiles,pi_g_specific))
	print ('Calculating Pi-G for all cells that passed filter for specific genes...')	
	cmds=[]
	genelist_file=os.path.join(o.experimentdir,'QC/genelist.txt')
	for cell in glob.glob(os.path.join(o.experimentdir,bamfiles,filterTaggedFiles,'*/*_taggedFiltered.bam')):
		cellname=cellNameExtration(cell)
		if cellListCheck(cellname,o.experimentdir):
			outfile=os.path.join(o.experimentdir,outfiles,pi_g_specific,cellname,cellname+'_pi_g.pkl')
			safe_mkdir(os.path.join(o.experimentdir,outfiles,pi_g_specific,cellname))
			if os.path.exists(os.path.join(o.experimentdir,qcfiles,p_e,cellname+'_p_e.txt')):
				pe_file=open(os.path.join(o.experimentdir,qcfiles,p_e,cellname+'_p_e.txt'))
				pe_estimate=pe_file.read().split('\n')[0]
			else:
				pe_estimate=None
			cmd=['python3', os.path.join(rootDir,'scripts/estimate_pi_g_STAN.py'),cell,outfile,pe_estimate,genelist_file,os.path.join(rootDir,'data/NASCseqModel.stan')]
			cmds.append(cmd)
	if verbose: print(" ".join(cmds))
	Parallel(n_jobs=int(o.numCPU))(delayed(run_cmd)(cmd) for cmd in cmds)
	for cmd in cmds:
		commandlogfile.write('%s\n' % cmd)
	print ("Pi-G has been calculated for all cells that passed filter for a set of specific genes.")


if o.flag=='calculateSpecificPE':
	p_e_specific='p_e_specific'
	print ('Calculating PE for specific genes, for all cells that passed filter...')
	genelist=os.path.join(o.experimentdir,qcfiles,errorRates,'geneList.csv')
	genelist_file=os.path.join(o.experimentdir,'QC/genelist_up.txt')
	print ('Genelist found in file %r' %genelist_file)
	cmds=[]
	for cell in glob.glob(os.path.join(o.experimentdir,bamfiles,filterTaggedFiles,'*/*_taggedFiltered.bam')):
		cellname=cellNameExtration(cell)
		if cellListCheck(cellname,o.experimentdir):
			outfile=os.path.join(o.experimentdir,qcfiles,errorRates,cellname+'_ErrorRates_Specific.csv')
			cmd=['python2', os.path.join(rootDir,'scripts/PECalculation_GJH_geneSpecific.py'),cell,outfile,genelist_file]
			cmds.append(cmd)
	if verbose: print(" ".join(cmds))
	Parallel(n_jobs=int(o.numCPU))(delayed(run_cmd)(cmd) for cmd in cmds)
	for cmd in cmds:
